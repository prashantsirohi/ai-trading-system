...
4. It delegates the actual ingest to `daily_update_runner.run(...)` with `symbols_only=True` (so features are skipped here) and the resolved fallback flag (`service.py:44-60`).
5. The runner picks NSE-primary (`_run_nse_yfinance_daily_update`) or Dhan-primary (`_run_dhan_primary_daily_update`) based on the `nse_primary` flag (`daily_update_runner.py:1317-1334`). NSE is the source of record per CLAUDE-level policy.
6. After ingest, the service queries `_catalog` via `fetch_catalog_summary` and computes `freshness_status` ∈ {`fresh`, `delayed`, `stale`} with `classify_freshness_status` (`service.py:62-77`, `:189-195`).
