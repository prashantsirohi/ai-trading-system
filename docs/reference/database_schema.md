...
Migration 009 adds `jcurve_research_run`, `jcurve_announcement`,
`jcurve_agent_request`, `jcurve_claim`, `jcurve_claim_review`, `jcurve_episode`,
`jcurve_episode_evidence`, and `jcurve_stage_observation`. Exact columns and
uniqueness constraints are owned by
`domains/research_screener/migrations/009_jcurve_research.sql`.
Migration 010 adds `jcurve_seed_run` and `jcurve_seed_candidate` for immutable
Screener screen/accounting discovery. Exact columns and constraints are owned
by `domains/research_screener/migrations/010_jcurve_screener_seed.sql`.
Migration 011 adds the frozen screen exchange, listing code, ISIN, NSE symbol,
and BSE code needed to audit and resolve BSE-only screen members.
Migration 012 adds `jcurve_discovery_run`, `jcurve_discovery_screen`, and
`jcurve_discovery_candidate` for the immutable four-screen union, official
universe disposition, accounting classification, and bounded research queue.
