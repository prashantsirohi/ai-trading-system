...
| `google_sheets_event_log` | `channels/google_sheets.py::publish_event_log_sheet` (`publish.py:460-473`) | `decision_bundle` (events) | Google Sheet | `publish_auxiliary` (**blocking**) | sha256(channel + rank hash + event_hashes) | 3, expo | Raises; skipped if `decision_bundle` is missing (`publish.py:469-470`) |
