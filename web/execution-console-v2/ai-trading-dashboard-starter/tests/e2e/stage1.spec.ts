...
test.beforeEach(async ({ page }) => {
  ...
  await page.route('**/api/execution/workspace/pipeline**', route => route.fulfill({ json: {} }));
  await page.route('**/api/execution/ranking**', route => route.fulfill({ json: { top_ranked: [] } }));
  await page.route('**/api/execution/tasks**', route => route.fulfill({ json: { rows: [] } }));
  ...
  await page.route('**/api/execution/investigator/stage1/READY/analytics-history', route => route.fulfill({ json: { metadata: { data_source: 'DUCKDB' }, rows: [
    { trade_date: '2026-07-10', stage1_maturity_score: 74, stage1_emerging_rank: 4, golden_cross_status: 'APPROACHING' },
    { trade_date: '2026-07-11', stage1_maturity_score: 78, stage1_emerging_rank: 2, golden_cross_status: 'IMMINENT' },
  ] } }));
});
...
test('filters breakout-ready candidates and opens lifecycle detail', async ({ page }) => {
  ...
  await expect(page.getByRole('cell', { name: '2026-07-11', exact: true })).toBeVisible();
  expect((page as typeof page & { browserErrors?: string[] }).browserErrors).toEqual([]);
