...
import type { SectorConstituent } from '@/lib/mock/sectorConstituents';
...
function chips(row: SectorConstituent): IndChip[] {
  return [
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-slate-800 text-left text-[10px] uppercase tracking-wider text-slate-500">
              ...
              <th className="pb-2 pr-4 text-right">MACD</th>
            ...
            {rows.map((row) => {
              ...
              return (
                ...
                >
                  ...
                  <td className={cn('py-2 pr-4 text-right font-mono', row.macd >= 0 ? 'text-emerald-400' : 'text-rose-400')}>
                    {row.macd >= 0 ? '+' : ''}{row.macd.toFixed(2)}
                  </td>
                  <td className="py-2 pr-4 text-right font-mono text-slate-300">{row.volMult.toFixed(1)}×</td>
                  <td className="py-2 pr-4 text-right font-mono text-slate-200">{row.score.toFixed(2)}</td>
                  <td className="py-2">
