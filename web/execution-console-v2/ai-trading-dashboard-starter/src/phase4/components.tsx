...
export function AppShell({ children, readiness }: { children: ReactNode; readiness?: JsonRecord }) {
  ...
  return <div className="operator-app"><a className="skip-link" href="#main-content">Skip to content</a><header className="topbar"><div><span className="eyebrow">AI Trading System</span><strong>Operator Workspace</strong></div><div className="top-status"><StatusBadge value="API connected" /><FreshnessBadge freshness={{ freshness_status: readiness ? 'FRESH' : 'UNKNOWN' }} /><button type="button" onClick={auth.signOut}>Sign out</button></div></header>{phase4View && !productionReady ? <div className="production-banner" role="alert"><strong>Phase 4 development view only — production readiness is blocked.</strong><span>{limitations.length || 4} active limitations</span><NavLink to="/readiness">View readiness</NavLink></div> : null}<div className="app-grid"><nav className="side-nav" aria-label="Primary">{navItems.map(([to, label]) => <NavLink key={to} to={to} end={to === '/' || to === '/trade-journal'}>{label}</NavLink>)}</nav><main id="main-content"><div className="page-tools"><span>Current view: {navItems.find(([to]) => to === location.pathname)?.[1] ?? 'Detail'}</span><button type="button" onClick={() => queryClient.invalidateQueries()}>Reload displayed data</button></div>{children}</main></div></div>;
}
