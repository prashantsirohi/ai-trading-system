...
export function OverviewPage() {
  ...
  return <div><PageHeader eyebrow="Operational overview" title="What needs attention now" description="API-projected status only. No strategy, ranking, routing, or lifecycle logic is recomputed here." meta={readiness.data?.meta} />
    ...
    <div className="overview-grid">
      ...
      <section className="section-card"><h2>Position monitoring</h2><Metric label="Active positions" value={positions.error ? null : positionRows.length} unavailable={Boolean(positions.error)} /><Metric label="Router-covered" value={countBy(positionRows, 'coverage_status', 'FULLY_MONITORED')} /><Metric label="Episode attached" value={positionRows.filter((row) => row.episode_attached === true).length} unavailable={!positionRows.some((row) => typeof row.episode_attached === 'boolean')} /><Metric label="Missing routing" value={countBy(positionRows, 'coverage_status', 'MISSING_ROUTING')} /><Metric label="Positive action suppressed" value={positionRows.filter((row) => row.positive_action_suppressed === true).length} /></section>
...
export function PositionsPage() {
  ...
  const configs: Record<string, { path: string; columns: Column[]; rowId: string }> = {
    coverage: { path: withParams('/api/v1/positions/coverage', params, ['coverage_status']), rowId: 'position_cycle_id', columns: [{ key: 'symbol_id', label: 'Symbol' }, { key: 'position_cycle_id', label: 'Position cycle' }, statusColumn('coverage_status', 'Router coverage'), boolColumn('route_data_covered', 'Route and data covered'), boolColumn('episode_attached', 'Episode attached'), { key: 'reconciliation_observed_session', label: 'Reconciliation session' }, statusColumn('effective_scan_tier', 'Scan tier'), boolColumn('market_data_complete', 'Market data complete'), boolColumn('evidence_complete', 'Evidence complete'), listColumn('missing_fields', 'Missing fields'), statusColumn('episode_compatibility', 'Episode compatibility'), boolColumn('positive_action_suppressed', 'Positive action suppressed'), statusColumn('recovery_status', 'Recovery')] },
