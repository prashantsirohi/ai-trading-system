...
import {
  ConflictBanner, DataTable, EmptyState, ErrorState, KeyValuePanel, LimitationList,
  PageHeader, PaginationControls, PartialDataBanner, ReadinessBadge, SeverityBadge,
  StatusBadge, Tabs, useDetailNavigation, type Column,
} from './components';
...
export function MarketPage() {
  ...
  return <div><PageHeader eyebrow="Market structure" title="Market, sectors, and stock stages" description="Historical stage selection is delegated to the API. Conflicted entities never receive a frontend-selected winner." meta={market.data?.meta ?? sectors.data?.meta} asOf />
    <section className="section-card"><h2>Market-stage summary</h2><QueryState query={market}>{(response) => { const data = asRecord(response.data); const conflicts = asRecords(data.conflicts); return <>{conflicts.length ? <ConflictBanner message={`${conflicts.length} governed conflicts are present; no false authoritative value is shown.`} /> : null}<DataTable rows={asRecords(data.observations)} columns={[{ key: 'symbol_id', label: 'Symbol' }, statusColumn('effective_stage', 'Stage'), statusColumn('stage_status', 'Status'), { key: 'as_of', label: 'As of' }]} rowKey={(row, index) => text(row.observation_id, String(index))} caption="Market stage observations" /></>; }}</QueryState></section>
    <section className="section-card"><h2>Sector stages</h2><div className="filters"><label>Stage<input value={params.get('sector_stage') ?? ''} onChange={(e) => { const next = new URLSearchParams(params); e.target.value ? next.set('sector_stage', e.target.value) : next.delete('sector_stage'); setParams(next); }} /></label><label>Status<input value={params.get('stage_status') ?? ''} onChange={(e) => { const next = new URLSearchParams(params); e.target.value ? next.set('stage_status', e.target.value) : next.delete('stage_status'); setParams(next); }} /></label></div><QueryState query={sectors}>{(response) => <><PartialDataBanner meta={response.meta} /><DataTable rows={asRecords(response.data)} columns={sectorColumns} rowKey={(row, index) => text(row.observation_id, String(index))} caption="Sector stages" /></>}</QueryState></section>
    <section className="section-card"><h2>Stock stage explorer</h2><p className="muted">Enter a symbol to query the server. The browser does not load the full universe.</p><label htmlFor="symbol-search">Symbol</label><input id="symbol-search" value={symbol} onChange={(e) => { const next = new URLSearchParams(params); e.target.value ? next.set('symbol', e.target.value.toUpperCase()) : next.delete('symbol'); setParams(next); }} />{symbol ? <QueryState query={stocks}>{(response) => <DataTable rows={asRecords(response.data)} columns={stockColumns} rowKey={(row, index) => text(row.observation_id, String(index))} caption="Stock stages" />}</QueryState> : <EmptyState title="Search for a stock" detail="Symbol-scoped requests preserve the API-only boundary and avoid loading all symbols into browser memory." />}</section>
  </div>;
}
...
export function CandidatesPage() {
  ...
  const filters = <div className="filters"><label>Symbol<input value={params.get('symbol') ?? ''} onChange={(e) => { const next = new URLSearchParams(params); e.target.value ? next.set('symbol', e.target.value.toUpperCase()) : next.delete('symbol'); next.delete('cursor'); setParams(next); }} /></label><label>Candidate state<input value={params.get('candidate_state') ?? ''} onChange={(e) => { const next = new URLSearchParams(params); e.target.value ? next.set('candidate_state', e.target.value) : next.delete('candidate_state'); next.delete('cursor'); setParams(next); }} /></label></div>;
...
export function PositionsPage() {
  ...
  return <div><PageHeader eyebrow="Position safety" title="Active-position monitoring" description="Coverage gaps, positive-action suppression, episode compatibility, and recovery proposals are read-only." meta={query.data?.meta} /><Tabs options={['coverage', 'missing-data', 'recovery']} /><QueryState query={query}>{(response) => <DataTable rows={asRecords(response.data)} columns={config.columns} rowKey={(row, index) => text(row[config.rowId], String(index))} caption="Position monitoring" onRow={tab === 'coverage' ? goDetail : undefined} />}</QueryState></div>;
}

export function AlertsPage() {
  const [params] = useSearchParams();
  const tab = params.get('tab') ?? 'alerts';
  const path = withParams(tab === 'incidents' ? '/api/v1/alert-incidents' : '/api/v1/alerts', params, ['severity', 'status']);
  ...
  return <CollectionPage title="Alerts and incidents" eyebrow="Operational attention" description="Open, resolved, and recurred incident evidence. No acknowledgement or resolution controls are present." path={path} allowed={[]} columns={columns} rowId="alert_id" detailPrefix={tab === 'incidents' ? '/incidents' : '/alerts'} poll controls={<Tabs options={['alerts', 'incidents']} />} />;
}
...
export function GovernancePage() {
  ...
  const [path, columns, rowId] = configs[tab] ?? configs.conflicts;
  const query = usePhase4Query<JsonRecord[]>(path);
  return <div><PageHeader eyebrow="Governance" title="Conflicts and correction authority" description="Conflicts remain explicit. This view never chooses an authoritative winner on the API's behalf." meta={query.data?.meta} /><Tabs options={['conflicts', 'corrections', 'impacts', 'membership', 'routing']} /><QueryState query={query}>{(response) => <>{tab === 'conflicts' && asRecords(response.data).length ? <ConflictBanner message={`${asRecords(response.data).length} governed conflicts require attention.`} /> : null}<DataTable rows={asRecords(response.data)} columns={columns} rowKey={(row, index) => text(row[rowId], `${tab}-${index}`)} caption="Governance evidence" /></>}</QueryState></div>;
}
