...
import { useStage1Analytics, useStage1Current, useStage1Detail, useStage1Exits, useStage1Summary, useStage1Transitions } from '@/lib/queries';
...
function DetailDrawer({ symbol, close }: { symbol: string; close: () => void }) {
  ...
  const analytics = useStage1Analytics(symbol);
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-black/60" onMouseDown={e => e.target === e.currentTarget && close()}><aside className="h-full w-full max-w-2xl overflow-y-auto border-l border-slate-700 bg-slate-950 p-5">
    ...
    {isLoading ? <CardSkeleton /> : error ? <ErrorStateView error={error.message} /> : !current ? <EmptyState message="No Stage-1 history is available for this symbol." /> : <div className="space-y-4">
      ...
      <SectionCard title="Persisted analytical history" description="Actual Stage-1 daily facts from DuckDB; lifecycle transitions are not used to reconstruct these values.">{analytics.isLoading ? <CardSkeleton /> : analytics.error ? <ErrorStateView error={analytics.error.message} /> : !analytics.data?.rows.length ? <EmptyState message="No analytical history is available for the approved model version." /> : <div className="overflow-x-auto"><table className="min-w-[900px] text-left text-xs"><thead className="uppercase text-slate-500"><tr>{['Date','Score','Emerging rank','Structural repair','Accumulation','RS acceleration','Base quality','Sector rotation','Pattern readiness','Golden Cross','Pivot distance','Eligible'].map(label => <th className="px-2 py-2" key={label}>{label}</th>)}</tr></thead><tbody>{analytics.data.rows.map(row => <tr className="border-t border-slate-800" key={String(row.trade_date)}><td className="px-2 py-2">{text(row.trade_date)}</td><td className="px-2">{number(row.stage1_maturity_score,1)}</td><td className="px-2">{number(row.stage1_emerging_rank)}</td><td className="px-2">{number(row.structural_repair_score,1)}</td><td className="px-2">{number(row.accumulation_score,1)}</td><td className="px-2">{number(row.rs_acceleration_score,1)}</td><td className="px-2">{number(row.base_quality_score,1)}</td><td className="px-2">{number(row.sector_rotation_score,1)}</td><td className="px-2">{number(row.pattern_readiness_score,1)}</td><td className="px-2">{text(row.golden_cross_status)}</td><td className="px-2">{pct(row.distance_to_pivot_pct)}</td><td className="px-2">{text(row.stage1_eligible)}</td></tr>)}</tbody></table></div>}</SectionCard>
