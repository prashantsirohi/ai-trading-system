...
export function mapBackendStockRow(row: BackendRecord): StockRow {
  ...
  return {
    ...
    patternSignalDate: optionalText(row.top_pattern_signal_date ?? row.signal_date ?? row.fresh_signal_date ?? row.last_seen_date),
    patternStartDate: optionalText(row.top_pattern_start_date ?? row.pattern_start ?? row.first_seen_date),
    patternEndDate: optionalText(row.top_pattern_end_date ?? row.pattern_end ?? row.last_seen_date),
