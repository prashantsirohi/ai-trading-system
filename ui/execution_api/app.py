...
from ui.services.control_center import get_recent_runs, get_run_details
...
class PipelineRunRequest(BaseModel):
    ...
    stages: list[str] = Field(default_factory=lambda: ["ingest", "features", "rank", "publish"])
...
def create_app() -> FastAPI:
    ...
    def execution_ranking(limit: int = Query(default=25, ge=1, le=200)) -> dict[str, Any]:
    ...
    def execution_task_logs(
        ...
        try:
            return get_task_snapshot(_project_root(), task_id, after=after, log_limit=limit)
        except KeyError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc

    @app.get("/api/execution/tasks/{task_id}/events")
    async def execution_task_events(task_id: str, cursor: int = Query(default=0, ge=0)) -> StreamingResponse:
        ...
        async def _stream() -> Any:
            ...
            while True:
                try:
                    payload = get_task_snapshot(project_root, task_id, after=next_cursor, log_limit=100)
                except KeyError as exc:
                    event = {"error": str(exc)}
                ...
                if logs:
                    next_cursor = max(int(row.get("log_cursor", next_cursor)) for row in logs)
                event = {
                    "task": payload["task"],
                    "logs": logs,
                    "cursor": next_cursor,
                ...
                if payload["task"].get("status") in {"completed", "completed_with_publish_errors", "failed", "terminated"}:
                    return
    ...
    def execution_publish_retry(request: PublishRetryRequest) -> dict[str, Any]:
        try:
            task = retry_publish_action(_project_root(), local_publish=request.local_publish)
...
def main() -> None:
    ...
    uvicorn.run("ui.execution_api.app:app", host=args.host, port=args.port, reload=False)
