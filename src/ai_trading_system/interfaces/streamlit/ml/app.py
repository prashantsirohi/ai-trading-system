...
PROJECT_ROOT_PATH = Path(ensure_project_root_on_path(__file__))

from ai_trading_system.platform.utils.env import load_project_env
from ai_trading_system.ui.execution_api.services import (
    ...
    with recipe_col:
        ...
        with st.form("builder_recipe_form"):
            ...
            to_date = st.text_input(
                "To Date",
                value=db_paths.research_static_end_date() if selected_recipe_row is None else str(selected_recipe_row.to_date),
            )
...
def _render_training_tab() -> None:
    ...
    default_to_date = db_paths.research_static_end_date()
...
def main() -> None:
    ...
    st.sidebar.markdown(
        "\n".join(
            [
                "Run directly with:",
                "`PYTHONPATH=. ./.venv/bin/streamlit run src/ai_trading_system/interfaces/streamlit/ml/app.py`",
            ]
        )
    )
    _render_active_deployment_banner()
