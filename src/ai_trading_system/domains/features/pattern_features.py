...
def compute_pattern_preconditions(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    if output.empty:
        for col in (
            ...
            output.loc[:, col] = pd.Series(dtype=float)
        return output
    ...
    output.loc[:, "consolidation_range_pct"] = rolling_range.mean()
    output.loc[:, "base_tightness"] = 1.0 / (1.0 + output["consolidation_range_pct"])
    ...
    if "atr_14" in output.columns:
        ...
        output.loc[:, "volatility_contraction"] = atr_series / atr_baseline.replace(0, np.nan)
    else:
        tr = pd.concat(
            [
                high - low,
    ...
    output.loc[:, "pullback_depth_pct"] = ((rolling_high - close) / rolling_high.replace(0, np.nan)) * 100.0

    resistance = high.rolling(10, min_periods=5).max()
    output.loc[:, "resistance_slope"] = resistance.diff(5) / 5.0
    return output
