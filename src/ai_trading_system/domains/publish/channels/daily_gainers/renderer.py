"""Render daily gainers HTML and optionally PDF."""
...
def render_html(context: Dict[str, Any]) -> str:
    ...
    enriched["summary_html"] = _render_markdown(str(context.get("summary_md") or ""))
    return template.render(**enriched, inline_css=inline_css)
...
def render(context: Dict[str, Any], output_dir: Path) -> Tuple[Path, Optional[Path], Optional[str]]:
    ...
    report_date = str(context.get("report_date") or "latest")
    stem = f"daily_gainers_{report_date}"
    ...
    except Exception as exc:  # noqa: BLE001 - weasyprint can raise OS-level native lib errors
...
def _render_markdown(markdown_text: str) -> str:
    try:
        ...
        return "<p>" + escape(markdown_text).replace("\n", "<br />") + "</p>"
