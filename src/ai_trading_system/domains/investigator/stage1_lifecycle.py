    ...
    *, run_date: str, config: Stage1LifecycleConfig | None = None,
    ...
    for symbol in universe:
        ...
        record.update({
            ...
            "stage1_score_delta_5d": round(score - previous_score, 12) if np.isfinite(score) and np.isfinite(previous_score) else np.nan,
            "stage1_score_delta_20d": round(score - previous_score, 12) if np.isfinite(score) and np.isfinite(previous_score) else np.nan,
