...
def test_execute_honors_risk_off_cash_only(monkeypatch, tmp_path: Path) -> None:
    ...
    dashboard_path.write_text(
        json.dumps(
            {
                "summary": {"data_trust_status": "trusted"},
                "market_regime": {"regime": "risk_off", "raw_regime": "risk_off"},
                "regime_profile": {
                    "name": "profile_C_cash_only",
                    "regime": "risk_off",
                    "max_exposure": 0.0,
    ...
    monkeypatch.setattr(
        "ai_trading_system.analytics.regime_detector.RegimeDetector.get_market_regime",
        lambda self: {"market_regime": "MEAN_REVERSION"},
    )
    ...
    assert captured["regime"] == "risk_off"
    assert result.metadata["effective_execution_capital"] == 0
