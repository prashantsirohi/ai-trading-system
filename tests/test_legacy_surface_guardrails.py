...
def test_dashboard_wrappers_delegate_to_ui_modules() -> None:
    dashboard_files = [
        _repo_root() / "dashboard" / "__init__.py",
...
def test_config_settings_is_compatibility_shim() -> None:
    source = (_repo_root() / "config" / "settings.py").read_text(encoding="utf-8")
    assert "Compatibility" in source
    assert "platform.config.settings" in source
    assert "duckdb.connect" not in source
    assert "sqlite3.connect" not in source
