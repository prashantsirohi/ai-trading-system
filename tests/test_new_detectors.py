...
def test_pocket_pivot_detects_volume_surge_above_prior_down_days() -> None:
    n = 60
    # Smooth uptrend
    closes = np.linspace(100.0, 130.0, n)
    volumes = np.full(n, 100_000.0)
    # Two down days INSIDE the 10-bar window before the breakout bar (n-1):
    closes[55] = closes[54] - 1.0
    volumes[55] = 200_000.0
    closes[56] = closes[55] - 0.5
    volumes[56] = 180_000.0

    # Final bar: up day, volume far above the highest prior down-day volume.
    closes[-1] = closes[-2] + 2.0
    volumes[-1] = 1_500_000.0

    extras = {"volume_ratio_20": np.full(n, 2.5), "volume_zscore_20": np.concatenate([np.zeros(n-1), [2.5]])}
    frame = _make_frame(closes, volumes=volumes, extras=extras)

    signals, stats = detect_pocket_pivot_signals(frame, config=PatternScanConfig())
    assert stats.confirmed_count >= 1
    assert signals[-1].pattern_family == "pocket_pivot"


def test_pocket_pivot_requires_uptrend() -> None:
    n = 60
    # Flat / no uptrend
    closes = np.full(n, 100.0)
    closes[-1] = 102.0  # one-day up move
    volumes = np.full(n, 100_000.0)
    volumes[20] = 500_000.0
    volumes[-1] = 2_000_000.0
    closes[20] = 99.0  # establish a down-day baseline
    frame = _make_frame(closes, volumes=volumes)
    signals, _ = detect_pocket_pivot_signals(frame, config=PatternScanConfig())
    # No prior 5% advance → no pocket pivot.
    assert signals == []
...
def test_darvas_box_detects_breakout_above_held_top() -> None:
    n = 30
    # Build a tight box: high holds at 105, low holds at 100, then breakout.
    box_top = 105.0
    box_bot = 100.0
    closes = np.array([102.0] * (n - 1) + [108.0])
    highs = np.array([box_top] + [box_top - 0.5] * (n - 2) + [110.0])
    lows = np.array([box_bot + 0.5] * 1 + [box_bot] + [box_bot + 0.4] * (n - 3) + [105.5])
    volumes = np.full(n, 100_000.0)
    volumes[-1] = 250_000.0  # surge on breakout
    extras = {"volume_ratio_20": np.full(n, 2.5), "volume_zscore_20": np.concatenate([np.zeros(n-1), [2.5]])}
    frame = _make_frame(closes, highs=highs, lows=lows, volumes=volumes, extras=extras)

    config = PatternScanConfig(darvas_min_box_bars=10, darvas_max_box_bars=25)
    signals, stats = detect_darvas_box_signals(frame, config=config)
    assert stats.candidate_count >= 1
    assert stats.confirmed_count >= 1
    sig = signals[-1]
    assert sig.pattern_family == "darvas_box"
    assert sig.breakout_level == pytest.approx(box_top, rel=0.01)


def test_darvas_box_rejects_when_top_broken_within_hold_window() -> None:
    # Top set 2 bars before the breakout — fewer bars than top_hold (3) →
    # detector must reject because the level hasn't proven it can hold.
    n = 25
    box_top = 105.0
    closes = np.full(n, 102.0)
    closes[-1] = 108.0
    highs = np.full(n, box_top - 1.0)
    highs[-3] = box_top  # top set only 2 bars before breakout
    ...
    config = PatternScanConfig(
        darvas_min_box_bars=10,
        darvas_top_hold_bars=3,
        darvas_bottom_hold_bars=3,
    )
    signals, _ = detect_darvas_box_signals(frame, config=config)
    assert signals == []
