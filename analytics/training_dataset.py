"""Prepared training dataset builder for research ML workflows."""
...
class TrainingDatasetBuilder:
    """Build reproducible OHLCV+feature training datasets for model training."""

    def __init__(self, project_root: str | Path, data_domain: str = "research"):
    ...
    def prepare(
        ...
        *,
        ...
        df = self._enrich_features(df, horizon=horizon)

        feature_cols = engine._feature_cols(df)
        ...
        metadata: Dict[str, Any] = {
            ...
            "label_positive_rate": float(df[target_col].mean()) if target_col in df.columns else None,
        }
    ...
    def _add_regime_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        regime = ordered.groupby("trade_date").agg(
            pct_above_50=("close", lambda series: 0.0),
        )
        regime["pct_above_50"] = ordered.groupby("trade_date").apply(
            lambda frame: float((frame["close"] > frame["sma50_for_regime"]).mean() * 100)
        )
        regime["pct_above_200"] = ordered.groupby("trade_date").apply(
            lambda frame: float((frame["close"] > frame["sma200_for_regime"]).mean() * 100)
        )
        regime["pct_up_20"] = ordered.groupby("trade_date").apply(
            lambda frame: float((frame["close"] > frame["close_20_back"]).mean() * 100)
        )
        regime["pct_up_50"] = ordered.groupby("trade_date").apply(
            lambda frame: float((frame["close"] > frame["close_50_back"]).mean() * 100)
        )
