...
class IngestStage:
    ...
    def _run_default(self, context: StageContext) -> Dict:
        ...
        payload.update(self._run_bhavcopy_validation(context, payload))
    ...
    def _run_bhavcopy_validation(self, context: StageContext, ingest_payload: Dict) -> Dict:
        ...
        catalog_df = self._load_catalog_close_frame(context=context, validation_date=validation_date)
        scope_symbols = self._resolve_validation_scope_symbols(ingest_payload, catalog_df)
        bhavcopy_df, bhavcopy_source = self._load_reference_close_frame(
            context=context,
    ...
    def _load_reference_close_frame(
        ...
        *,
        ...
        if source_mode in {"auto", "bhavcopy"}:
            bhavcopy_df, bhavcopy_source = self._load_bhavcopy_close_frame(
                context=context,
                validation_date=validation_date,
            )
        ...
        if source_mode in {"auto", "yfinance"}:
            yfinance_df, yfinance_source = self._load_yfinance_close_frame(
                validation_date=validation_date,
    ...
    def _load_bhavcopy_close_frame(self, context: StageContext, validation_date: str) -> tuple[pd.DataFrame, str]:
        ...
        if source_path:
            ...
            if not csv_path.exists():
                raise DataQualityCriticalError(
                    f"Bhavcopy validation file not found: {csv_path}"
                )
    ...
    def _load_yfinance_close_frame(self, *, validation_date: str, symbol_ids: list[str]) -> tuple[pd.DataFrame, str]:
        ...
        for start_idx in range(0, len(symbol_ids), batch_size):
            batch_symbols = [str(symbol).strip().upper() for symbol in symbol_ids[start_idx : start_idx + batch_size] if str(symbol).strip()]
